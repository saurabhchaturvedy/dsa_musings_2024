package Learnings.WM202409.Java8;

import java.util.*;
import java.util.stream.Collectors;

public class Java8BrushUp {
    ...
    public static void main(String[] args) {
        ...
        employeeList.stream().map(Employee::getDepartment).distinct().forEach(e -> {
        ...
        Optional<Employee> collect2 = employeeList.stream().collect(Collectors.maxBy(Comparator.comparingDouble(Employee::getSalary)));
        ...
        Set<Map.Entry<Boolean, List<Employee>>> entrySett = partitionEmployeesByAge.entrySet();

        for (Map.Entry<Boolean, List<Employee>> entry : entrySett) {
            System.out.println("----------------------------");
